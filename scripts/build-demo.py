...
KARTLAR = [
    ...
    dict(src="stage-separation-preset", repo="stage_separation", kat="orbital",
         ad="Kademe Ayrılması", status="guncel",
         embed="../presets/stage_separation/index.html?kamera=sinema",
         hook="Roket kademe atlar: iki katı yakıtlı itici 69 s'de tükenip takla atarak ayrılır, MECO'dan sonra çekirdek SAHNEYE BIRAKILIR ve kendi balistiğini yaşarken üst kademe vakum motoruyla devam eder.",
         feats=["Zaman çizelgesi elle yazılmaz: Max-Q, yan itici tükenmesi/ayrılması, MECO, ayrılma, SES, kapak ve SECO gerçek tırmanış çözümünden türer",
                "Katı itici kısılamaz ve söndürülemez: modelde sabit akış + sabit itki; yüksek T/W için pitch programı yeniden ayarlandı (196 × 204 km)",
                "Bırakılan gövde KAPALI FORM balistikle çözülür — zaman sürgüsü ve ?t= herhangi bir ana adım atmadan gider",
                "Kapak iki yarım hâlinde açılıp dönerek uzaklaşır; ayrılma itkisi 2 m/s, bağıl ivme −a₂",
                "Sinema yönetmeni kesme noktalarını olaylardan alır; ?sinema=1 arayüzü kaldırır, 2.39:1 maskesi iner"]),
    dict(src="mechanism-lab-preset", repo="mechanism_lab", kat="orbital",
         ad="Aksam Laboratuvarı", status="guncel",
         embed="../presets/mechanism_lab/index.html?mekanizma=rover.rockerBogie",
         hook="Bir aksamı söküp bakmak: parçalar montaj doğrultusunda ayrılır, pivotlar ve dönme eksenleri çizilir, eklemin taradığı gerçek yol görünür — ve her mekanizmanın yanında onu sınayan doğrulama adımı yazar.",
         feats=["16 mekanizma: rocker-bogie, Ackermann direksiyon, çıtalı tekerlek, robot kol, pan-tilt direk, TVC gimbal, kafes kanatçık, SRB ayrılması, yaylı açılım, eş eksenli rotorlar…",
                "Patlatılmış görünüm + kinematik iskelet (pivot küreleri, eksen okları) + hareket zarfı (sürülen düğümün en uç parçasının GERÇEK yolu)",
                "Künye: yöneten bağıntı · neden böyle · hangi doğrulama sınıyor; serbestlik derecesi rig haritasından TÜRETİLİR",
                "Her eklem elle sürülebilir (sınır, hız, tek yönlü/yay bilgisiyle); ?mekanizma= ve ?t= ile derin bağlantı"]),
    dict(src=None, repo="satellite_integration", kat="orbital",
         ad="Uydu Entegrasyonu", status="guncel",
         embed="../presets/satellite_integration/index.html?patla=60",
         links=[("Sayfayı aç ↗", "../presets/satellite_integration/index.html"), ("İtki adımı", "../presets/satellite_integration/index.html?adim=2&patla=45"), ("Tam patlatma", "../presets/satellite_integration/index.html?patla=100")],
         hook="Bir uydunun neye benzediğini değil, nasıl kurulduğunu gösterir: hangi parça neye cıvatalanır, hangi sırada takılır ve neden o sırada.",
         feats=["33 parça türü, 43 adet, 8 alt sistem, 7 montaj adımı; her parçada arayüz türü, bağlanma zinciri, kütle ve NEDEN VAR olduğu",
                "Montaj sırası fiziksel kısıttır: tank panel kapanmadan girer, ekipman panele tezgâhta takılır, MLI en son gelir (altındaki cıvatalara erişim biter)",
                "Patlatma yönü montajın TERSİDİR ve parçanın ebeveynine göre konumundan türetilir — parçalar rastgele dağılmaz",
                "Kütle bütçesi toplanır (709,4 kg kuru + 415 kg itici) ve kütle merkezi ayırma halkası ekseninden 32,7 mm sapar"]),
    dict(src=None, repo="light_blocks", kat="orbital",
         ad="Işık Fiziği", status="guncel",
         embed="../presets/light_blocks/index.html?ortam=mars",
         links=[("Sayfayı aç ↗", "../presets/light_blocks/index.html"), ("Vakum", "../presets/light_blocks/index.html?ortam=vakum"), ("İç mekân", "../presets/light_blocks/index.html?ortam=ic")],
         hook="Aynı lamba dört ortamda yan yana: fark süsleme değil, içeriğin kendisi. Ortam kuralı bağlayıcıdır ve her reddi gerekçesiyle söyler.",
         feats=["Renk üç meşru kaynaktan gelir: sıcaklık (Planck → CIE 1931 → doğrusal sRGB, yayımlanmış eğriye Δxy < 0,0016), kimya (emisyon hatları) ve yansıma (albedo × aydınlatan)",
                "Vakumda açık alev yanmaz, ışık huzmesi görünmez, duman asılı kalmaz — üçü de reddedilir ve yerine ne konacağı önerilir",
                "Dolgu ışığı HESAPLANIR, AmbientLight hiç kurulmaz: regolit sıçraması E·ρ·sin(irtifa)/π, Dünya ışığı ~12 lux, Ay yüzeyinde Güneş 127 000 lux",
                "Far kandela ile tanımlı, zemin aydınlatması E = I/d² olarak ekranda ölçülü; pembe gürültü PSD'si ölçülen eğimle −1,018"]),
    dict(src=None, repo="exploded_view", kat="orbital",
         ad="Patlatılmış Görünüm", status="guncel",
         embed="../presets/exploded_view/index.html?object=satellite",
         links=[("Sayfayı aç ↗", "../presets/exploded_view/index.html"), ("Yüzey üssü", "../presets/exploded_view/index.html?object=habitat"), ("Gezgin (türetilmiş)", "../presets/exploded_view/index.html?object=rover")],
         hook="Tek bir montaj çekirdeği, altı farklı cisim: beyan edilmiş katalogla türetilmiş sahne grafiği yan yana, ikisi de aynı yoldan patlıyor.",
         feats=["Patlatma yönü rastgele değil: her parça BAĞLANDIĞI parçadan, montajın tersi doğrultuda çıkar; aynı doğrultudakiler izdüşüme göre paketlenir, hiçbiri diğerini geçmez",
                "Hareket modeli saf ve three'siz: kademe (dış parçalar önce kalkar), smootherstep yumuşatma ve ayrılma açısı (k, parça)'nın kapalı formları — kaydırağı geri sürmek tam olarak geri izler",
                "Her parça gidiş yönüne DİK eksende döner ve varışta beyan edilmiş duruşuna tam döner (ölçülen 3e−17 rad): patlatılmış hâl bir çizimdir",
                "Türetilen cisimlerde kütle sınır hacminden TAHMİN edilir ve arayüzde her yerde öyle işaretlenir — beyan edilenle karıştırılmaz"]),
    dict(src=None, repo="habitat_blocks", kat="orbital",
         ad="Habitat Blokları", status="guncel",
         embed="../presets/habitat_blocks/index.html?ortam=mars",
         links=[("Sayfayı aç ↗", "../presets/habitat_blocks/index.html"), ("Ay ortamı", "../presets/habitat_blocks/index.html?ortam=moon")],
         hook="Dört mürettebatlı bir yüzey üssü: 26 bileşen, 7 kurulum adımı ve ortama uymayan bileşeni gerekçesiyle reddeden bir kural.",
         feats=["Ortam kuralı bağlayıcıdır: Ay'da MOXIE (atmosferde CO₂ yok) ve rüzgâr ölçer (ölçülecek rüzgâr yok) reddedilir ve 428 kg bütçeden düşer",
                "Dört bütçe kapanmak zorunda — asıl soru gündüz değil FIRTINA: panel %20'ye düşüp sera ile ISRU kapandığında kritik 5,81 kW yükü yalnız fisyon taşır (%45 pay)",
                "Hat çizilmez, hesaplanır: mesnet aralığı sarkma sınırından çıkar ve g^(−1/3) ile ölçeklenir (Ay 13,3 m, Mars 10,1 m); sarkma zincir eğrisidir, parabol değil",
                "Sabatier'in su/metan oranı stokiyometriden sınanır: ölçülen 2,250, beklenen 2,246"]),
    dict(src=None, repo="life_signs", kat="orbital",
         ad="Nefes Kütüphanesi", status="guncel",
         embed="../presets/life_signs/index.html?t=128",
         links=[("Sayfayı aç ↗", "../presets/life_signs/index.html"), ("Yakın plan", "../presets/life_signs/index.html?kamera=yakin&debug=1"), ("Ölçüm", "../presets/life_signs/index.html?odak=roverBeacon&t=96")],
         hook="Bir sahne \"canlı\" olsun diye hareket eklenmez. Gerçek bir mekanizmanın gerçek boşta davranışı gösterilir: duran gezgin gövdesiyle salınmaz, direğini çevirir — hız sınırıyla.",
         feats=["Tek sinüs ASLA: her genlik iki ölçüşmez sinüsün çarpımı olan pürüz zarfıyla modüle edilir; denetim desenin tekrar periyodunu sürekli kesirle ölçer (> 120 s)",
                "Hız sınırı aktüatör fiziğinden gelir: kapalı form kritik sönümde ω = min(ωMax, rateMax·e/|e₀|) — büyük adım daha uzun sürer, direk 12°/s'yi aşmaz",
                "Dikkat bütçesi: en çok 3 fark edilir davranış, Σ salience ≤ 1,0; kamera yaklaştıkça bütçe DARALIR",
                "Yasak sınıflar reddedilir: arazi, alan çizgileri, yörünge izi, sabit ışıkta gölge ve boştaki HUD sayısı nefes almaz"]),
    dict(src="physical-rigs-preset", repo="physical_rigs", kat="orbital",
         ad="Fiziksel Aksam", status="guncel",
         embed="../presets/physical_rigs/index.html?kamera=takip",
         hook="Gezgin gerçek araziye oturur: altı tekerleğin altında zemin örneklenir, rocker-bogie tek geçişte çözülür, gövde iki rockerın ORTALAMASINDA durur — ölçülen temas artığı 1,9 mm.",
         feats=["Kesintisiz dönen zemin: yükseklik z'de periyodik; döşeme sınırında ne yükseklik ne EĞİM atlar (1,8e−15 / 1,6e−14)",
                "Döngü tekerleğin tam sayı devrinden kapanır: travel = 2π·r_dış·28 — çıtalar döngü sınırında sıçramaz",
                "Yuvarlanma yarıçapı ÇITA DIŞ KÖŞESİ; kayma ω = v/(r(1−s)), 25° üstünde rig ilerlemeyi reddeder (patinaj)",
                "Tekerlek izi aracın GEÇMİŞ yolunu izler; rotor 80 Hz çıta frekansında bulanıklık diskine döner"]),
    dict(src=None, repo="site_plan", kat="orbital",
         ad="Saha Yerleşimi", status="guncel",
         embed="../presets/site_plan/index.html?seed=20260924",
         links=[("Sayfayı aç ↗", "../presets/site_plan/index.html"), ("Mars sahası", "../presets/site_plan/index.html?ortam=mars&seed=7"), ("Arazide", "../presets/site_plan/index.html?gorunum=arazi")],
         hook="Bir üste hiçbir mesafe keyfi değildir: her biri bir kazayı ya da bir kaybı önler. Kurallar adlandırılır, yerleşim onlara göre çözülür ve her kuralın ÖLÇÜLEN değeri hedefinin yanında durur.",
         feats=["11 kural: reaktör ≥ 1 km, pist ≥ 500 m ve ±30° ejecta konisinde yapı yok, yakıt ↔ O₂ ≥ 50 m, radyatör Güneş'e sırt, garaj ≤ 150 m, ISRU rüzgâr üstü (Mars)",
                "Sert kural adayı ELER, yumuşak kural ağırlıkla puanlar; sağlanamayan modül yine konur ama 'zorlandı' diye bildirilir — sessiz kabul yok",
                "Modülü sürükle: konum elle sabitlenir, kurallar anında yeniden ölçülür; aynı tohum → aynı yerleşim",
                "Çözümün kapıları araziye GERİ yazılır: saha çokgenleri ve 24 m koridorlar zemini kendi yerel kotasına tesviye eder"]),
    dict(src=None, repo="terrain_blocks", kat="orbital",
         ad="Arazi Blokları", status="guncel",
         embed="../presets/terrain_blocks/index.html?profil=moon-highland&seed=20260916&sun=14",
         links=[("Sayfayı aç ↗", "../presets/terrain_blocks/index.html?profil=moon-highland"), ("Mars kanyonu", "../presets/terrain_blocks/index.html?profil=mars-canyon&sun=22"), ("Eğim haritası", "../presets/terrain_blocks/index.html?profil=moon-mare&view=slope&cam=bird")],
         hook="Alelade düz zemin yerine adlandırılmış olgular: Pareto boy dağılımlı krater alanı, havza masifleri, mare sırtları, kanyon, kumul, mesa — ve tekerleğin, ayağın, gölgenin sorgulayabildiği analitik bir yüzey.",
         feats=["h(x,z) = gerçek yarıçaplı sagitta + Σ kapı·katman; 1 birim = 100 m, abartı kilitli 1 (açılırsa ilan edilir)",
                "Regolit 35° durma açısına termal erozyonla kırpılır (sert kaya muaf); düzeltme analitik alana geri yazılır — örgü ile sorgu < 1 mm",
                "Lambert regolit: eğim/yükseklik albedo, dört-tap detay merdiveni, krater ışınlı bölge albedosu; mavi-gürültü kaya/çakıl yerel normale yatık",
                "Profiller Ay mare · yayla · kutup kenarı, Mars ova · kanyon, genel kayalık; Node doğrulaması: determinizm, sagitta, ped, koridor, Pareto −2, Ay'da kumul reddi"]),
    ...
    dict(src="three-body-states-preset", repo="three_body_states", kat="orbital",
         ...
         feats=["Šuvakov–Dmitrašinović (2013) eşit-kütle aileleri: figure-8, butterfly I–III, bumblebee, moth I–III, goggles, dragonfly, yarn, yin-yang I/II; başlangıç koşulları literatürden",
                ...
                "RK4 uyarlanır adım (h ∝ d_min^1.5); her çözüm denetimde bir periyot sonra başlangıca döner (≤ 2e-3), enerji ve açısal momentum korunur",
                "Işıyan cisimler, sönen kuyruklu izler, tam periyot yolu; toplamsal ışık, dekor yok; ?t=&speed=&labels=1 deterministik"]),
    dict(src="soi-explorer-preset", repo="soi_explorer", kat="orbital",
         ad="Etki Küresi Kâşifi", status="cekirdek",
         embed="../presets/soi_explorer/index.html",
         hook="r_SOI = a(m/M)^(2/5): Dünya'nın 924 647 km'lik etki küresi, Ay'ınki ve Hill küresi gerçek ölçekte; park yörüngesinden v∞ hedefli hiperbol kabuğa varır, çerçeve el değiştirir — yamalı-konik artığı sayıyla.",
    ...
    dict(src="free-return-preset", repo="free_return", kat="orbital",
         ...
         hook="LEO'dan TLI ΔV ve faz açısı taramasıyla Dünya–Ay CR3BP'de manevrasız geri dönen 8 figürü: perilune ~1 000 km, dönüş perigee 100 km, ~6 gün — Apollo 13'ün yolu.",
         feats=["core/astro-cr3bp adaptif adımlı RK4 (Jacobi sapması 1e−6); LEO → dönen çerçeve dönüşümü v_R = v_I − ω × r; perilune ve dönüş perigee sayısal",
                "(ΔV, θ₀) ızgara taraması + satır başına bisection: dönüş perigee 100 km çözümleri; ΔV 3,13–3,16 km/s, Ay'a ~3 gün, toplam ~6–7 gün (denetimde)",
                "Dar koridor: θ₀'da 0,3° sapma perigee'yi > 500 km kaydırır — TLI zamanlaması hassasiyeti ölçülür",
                "Dönen ve eylemsiz çerçeve görünümleri, tarama haritası (renk: dönüş perigee; kırmızı: Ay'a çarpma); ?dv=&th= deterministik"]),
    dict(src="launch-window-preset", repo="launch_window", kat="orbital",
         ad="Fırlatma Penceresi ve Azimut", status="cekirdek",
         embed="../presets/launch_window/index.html",
         hook="sin β = cos i / cos φ: sahadan hedef düzleme azimut, Dünya dönmesi düzeltmesi, günde iki fırsat (LST) ve gecikme → düzlem değişimi ΔV → pencere genişliği.",
         feats=["Küresel trigonometri (Vallado 6.4): eylemsiz azimut, düğüm uzaklığı λ_u, LST = Ω + λ_u → UTC; KSC→ISS 45,0°/135,0° denetimde",
                "Dönme düzeltmesi: v_rel = V_bo(sin β, cos β) − (ω_e R cos φ, 0) → uçulan azimut 42,8°, tasarruf ~280 m/s (ekvator 465 m/s)",
                "Gecikme cezası cos θ = cos²i + sin²i cos(ω_e Δt), ΔV = 2V sin(θ/2); 100 m/s bütçeyle ISS penceresi ±3,8 dk; menzil güvenliği sektörü",
                "6 saha (KSC, Vandenberg, Baykonur, Kourou, Tanegashima, Sinop kavramsal) × 5 hedef; i < φ ise dogleg uyarısı; ?site=&target=&raan=&doy=&dv= deterministik"]),
    dict(src="geo-stationkeeping-preset", repo="geo_stationkeeping", kat="orbital",
         ad="GEO İstasyon Tutma Bütçesi", status="cekirdek",
         embed="../presets/geo_stationkeeping/index.html",
         hook="Doğu–batı sürüklenme gerçek C̄22/S̄22'den, kuzey–güney Ay+Güneş 1 yıllık RK4'ten: kararlı 75° D / 105° B, ~1,7 m/s/yıl DB, ~46 m/s/yıl KG, 15 yıl yakıt kimyasal vs elektrikli.",
         feats=["gravity_field'in gerçek EGM96 tesseral katsayılarından teğetsel ivme; denge boylamları kök olarak bulunur (kararlı 75,1° D / 104,9° B — literatürle denetimde)",
                "orbit_perturbations üçüncü-cisim modeliyle eğiklik vektörü izi: Δi ≈ 0,86°/yıl → ΔV_KG = V_geo·Δi ≈ 46 m/s/yıl (literatür 0,75–0,95°/yıl)",
                "Libration periyodu (≈ 2,2 yıl), serbest sürüklenme λ(t), SRP eksantriklik çemberi (sayısal), Tsiolkovsky ömür yakıtı (Isp 300 vs 1500)",
                "Boylam/ömür/kütle kaydırıcıları, hazır istasyonlar (42° D Türksat, kararlı/kararsız noktalar); ?lon=&years=&m0= deterministik"]),
    dict(src="entry-dispersion-preset", repo="entry_dispersion", kat="orbital",
         ad="Giriş Dağılımı (Monte Carlo)", status="cekirdek",
         embed="../presets/entry_dispersion/index.html",
         hook="İniş noktası bir sayı değil bir dağılımdır: γ/v/ρ/L/D/kütle/yatış sapmaları → menzil histogramı, 3σ, duyarlılık payları ve doğrusal RSS ↔ Monte Carlo karşılaştırması.",
         feats=["reentry_corridor çekirdeğini yeniden kullanır (US76 + termosfer, Sutton–Graves); tohumlu Gauss sapmalar, 100–500 örnek",
                "Tek-değişken merkezi-fark duyarlılıkları ∂s/∂x; RSS σ_s ile Monte Carlo σ_s oranı doğrusallık ölçüsü (denetimde 0,7–1,3)",
                "Senaryolar: LEO nominal/dik, Ay dönüşü (yatış 60°), yalnız seyrüsefer, yalnız atmosfer, balistik sonda; işaretler denetimde (sığ γ ↑, yoğun ρ ↓, L/D ↑)",
                "Yörünge yelpazesi + iniş noktaları, histogram (p05/p50/p95, ±3σ), duyarlılık çubukları, menzil–γ saçılımı; ?s=&n=&seed= deterministik"]),
    dict(src="tisserand-graph-preset", repo="tisserand_graph", kat="orbital",
         ad="Tisserand Grafiği", status="cekirdek",
         embed="../presets/tisserand_graph/index.html",
         hook="Çoklu kütleçekim yardımı tasarımının haritası: gezegen sabit-v∞ eğrileri, geçişte eğri boyunca kayış (|Δα| ≤ δ_max) ve VEEGA gibi dizilerin geometrik erişilebilirliği.",
         feats=["T = a_P/a + 2√(a/a_P(1−e²)) geçişte korunur, v∞ = V_P√(3 − T); eğri boyunca T sabitliği ve tersinirlik denetimde",
                "δ_max = 2 asin(1/(1 + r_p v∞²/μ_P)) ile erişilebilir yay; Dünya rezonans eğrileri (a = a_E(k/m)^{2/3})",
                "Açgözlü dizi planlayıcı: VEEGA 3,6 km/s kalkıştan Jüpiter'e (varış v∞ ≈ 5,9 km/s); erişilemez diziler işaretlenir",
                "(r_a, r_p) log-log grafik + güneş-merkezli bacak elipsleri + bacak tablosu; fazlama yok (tarihler için porkchop); ?seq=&vinf0=&hmin= deterministik"]),
    dict(src="low-thrust-transfer-preset", repo="low_thrust_transfer", kat="orbital",
         ad="Düşük İtkili Transfer Laboratuvarı", status="cekirdek",
         embed="../presets/low_thrust_transfer/index.html",
         hook="Elektrikli itki spirali: ΔV Hohmann'ın üstüne çıkar, yakıt kat kat azalır — sayısal spiral Edelbaum analitik limitiyle ±0,5 % örtüşür.",
         feats=["Sürekli teğetsel itki, değişken kütle RK4 (adım = periyot/200); ΔV = ∫T/m dt; hedef yarı-büyük eksende durur",
                "Edelbaum ΔV = √(v₀² + v₁² − 2v₀v₁cos(πΔi/2)) ve yaw kanunu; Hohmann ve Tsiolkovsky karşılaştırması (Isp 1800 vs 320)",
                "Senaryolar: LEO→GEO eş-düzlem / Δi 28,5° / gölgeli, CubeSat 400→800 km, iyon MEO, 5 N kargo; itki, Isp, Δi, gölge ayarlanır",
                "Spiral (itki açık/kapalı renkli) + a(t), ΔV(t), m(t) grafikleri; e_max ve T/W ile varsayımın zorlandığı yer adlandırılır; ?s=&t= deterministik"]),
    dict(src="orbit-determination-preset", repo="orbit_determination", kat="orbital",
         ad="Yörünge Belirleme (EKF) Laboratuvarı", status="cekirdek",
         embed="../presets/orbit_determination/index.html",
         hook="Yer istasyonu menzil/menzil-hızı ölçümlerinden genişletilmiş Kalman filtresi: yakınsama, 3σ zarfı, NEES/NIS tutarlılığı, gözlenebilirlik ve model hatası.",
         feats=["Gerçek: iki-cisim + J2 RK4; istasyonlar döner, yükseklik maskesi, tohumlu Gauss gürültü (σρ 10 m, σρ̇ 1 cm/s)",
                "EKF: RK4 haritasının sonlu-fark STM'si, sürekli beyaz-ivme Q, analitik H (menzil, menzil-hızı), Joseph biçimi; NEES = eᵀP⁻¹e ve NIS denetimde (beklenen 6 ve 1)",
                "Senaryolar: tek/üç istasyon, SSO kutup, yalnız menzil / yalnız menzil-hızı, GEO zayıf gözlenebilirlik, Molniya, 'filtre J2'yi bilmiyor' (sapma) ve büyük başlangıç hatası (aşırı güven)",
                "Sahne: ECI izdüşümü + 3σ elipsi, konum/hız hatası ve 3σ zarfı (log), normalize yenilikler, NEES/NIS serisi; ?s=&t= deterministik"]),
    dict(src="halo-manifolds-preset", repo="halo_manifolds", kat="orbital",
         ad="Halo Yörüngeleri ve Değişmez Manifoldlar", status="cekirdek",
         embed="../presets/halo_manifolds/index.html",
         hook="Richardson tahmini + 6×6 STM düzeltmesiyle halo ailesi; monodromi özvektörlerinden kararlı/kararsız manifold tüpleri — Dünya–Ay ve Güneş–Dünya L1/L2.",
    ...
    dict(src="transfer-explorer-preset", repo="transfer_explorer", kat="orbital",
         ...
         hook="r1, r2, Δθ ve TOF'tan transfer yayı, v1/v2 ve ΔV bütçesi: Lambert'in anatomisi, kısa/uzun yol ve TOF taramasının Hohmann'a inişi.",
         feats=["Paylaşılan evrensel-değişken Lambert (porkchop ile aynı çözücü); Kepler yayı uç noktası r2'ye ~1e−7 km",
                "ΔV₁ = |v1 − v_c1|, ΔV₂ = |v_c2 − v2| okları, Δθ yayı, a/e/r_p/r_a; Δθ = 180° limitinde Hohmann'a 1e−4 eşleşme denetimde",
                "Log-TOF taraması: kısa ve uzun yol ΔV eğrileri, Hohmann kesikli referansı, tarama minimumu düğmesi",
                "Presetler: LEO→GEO (180°, 150°), Dünya→Mars (180°, 120°), Dünya→Jüpiter, Dünya→Venüs; ?p=&tof=&dth=&dir= deterministik"]),
    dict(src="gravity-field-preset", repo="gravity_field", kat="orbital",
         ad="Küresel Harmonik Yerçekimi Alanı", status="cekirdek",
         embed="../presets/gravity_field/index.html",
         hook="Derece/mertebe, kesme ve tek-derece katkısı: jeoit ve anomali haritası tam normalize Legendre ile hesaplanır — düşük derece gerçek, üstü açıkça SENTETİK.",
         feats=["U = (GM/r)Σ(R/r)^l ΣP̄_lm(C̄ cos mλ + S̄ sin mλ); kararlı özyineleme; ortonormallik ve C̄20 = −J2/√5 denetimde",
                "C̄20, C̄22/S̄22, C̄30, C̄40 EGM96'dan (yuvarlatılmış); geri kalanı Kaula 1e−5/l² sentetik — sahne her karede uyarır",
                "Jeoit N (Bruns, GRS80 normal alanı çıkarılmış) ve δg; küre üstü boyama + abartılı yer değiştirme; kesme L, l_min, yalnız zonal, tek derece",
                "Derece varyans spektrumu vs Kaula; yalnız-C̄20 alanında sayısal düğüm kayması J2 analitikle çapraz-denetlenir"]),
    # ── ML ──
