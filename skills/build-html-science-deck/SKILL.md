...
For between-slide motion, integrate `design-scientific-motion/presets/motion_core/slide-transitions.js` in place of a bare active-class toggle and keep one transition grammar per deck.
...
Prefer relative asset paths. Self-host required fonts for offline delivery. Do not expose secrets in frontend code. Run `scripts/validate-deck-manifest.mjs` for structured manifests and use `assets/static-starter/index.html` as the minimal runtime reference. Before shipping, also run `$create-scientific-visuals/scripts/validate-icon-usage.mjs <deck.html>` — a `#i-` reference with no sprite behind it renders as an invisible empty box that nobody notices until the room does.
