...
For explaining an EXISTING equation term by term, read `references/equation-steps-preset.md` and use `/presets/equation-steps-preset/`: the equation stays fully typeset while each step focuses a set of terms (accent underline, rest ghosted) with an assertion caption — the derivation-walkthrough complement of the pen preset.

For guided attention on a figure or image, read `references/figure-callout-preset.md` and use `/presets/figure-callout-preset/`: percent-coordinate boxes/circles/arrows that trace in step by step, an optional spotlight scrim, a magnifier lens, and assertion captions with keyboard navigation.
