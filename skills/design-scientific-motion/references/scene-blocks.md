...
| `exploded_view` | **wave 2 · F0–F2** | The GENERIC exploded-view system — one core, many objects. `core/assembly.mjs` is three-less, so every explosion decision is verifiable without a screen: direction is the reverse of assembly and is derived from each part's position relative to its PARENT (not the group centre); parts travelling the same way are packed by their projection along that direction with a forward pass that preserves order, so none overtakes another (packing by depth let a deeper part pass a shallower one — measured 16 mm clearance at k=0.5); the root stays put, otherwise the whole body translates. Four modes (assembly/axial/radial/layered), depth staging, step playback, clipping section, picking. A malformed `axis` now THROWS instead of silently producing zero offsets (`axis: 'z'` made axial and layered move 0/26 parts). Two adapters: `adapters/catalog.mjs` for declared catalogues (satellite and surface base share the exact same shape), `adapters/scene.mjs` for ANY three object — it derives the tree from the scene graph, ESTIMATES mass from bounding volume, stamps every estimate `tahmini: true`, and synthesises a zero-mass root when the scene has several rootless nodes. Labels never sit on the body: they go to two gutters, are distributed by MEASURED height and joined to their part by a leader line. The motion layer is separate and equally PURE: `core/exploded-motion.mjs` never imports three, and stagger (outermost parts leave first), smootherstep easing, separation attitude, speed and the selection pulse are all closed forms of (k, part) - no integrator, no per-frame state, so scrubbing the slider back retraces exactly. Each part turns about an axis PERPENDICULAR to its travel and **returns to its declared attitude exactly on arrival** (measured 3e-17 rad): the exploded state is a drawing, and a part left tilted there defeats it. Per-part variation comes from a hashed id, never Math.random; the hash needed a MurmurHash3 finalizer because FNV-1a alone put similar ids within 0.01 of each other (chi-square 128.9 against a 1% threshold of 21.67, now 8.0). Turntable, hover/selection highlight and damped camera transitions live in the showcase, whose language is English. `scripts/validate-assembly.mjs` (52 checks) and `scripts/validate-exploded-motion.mjs` (33 checks) |
| `habitat_blocks` | **wave 2 · F0–F1** | Surface-base components with a BINDING environment rule. The catalogue shape is identical to `sat-parts.mjs`, which is the point: anything that declares those fields explodes, budgets and validates through the same path. On the Moon, MOXIE (no atmospheric CO₂) and the wind sensor (no wind to measure) are REFUSED with a reason and a suggested replacement, and their 428 kg leaves the mass budget — a budget that ignores the environment was reporting 26 components' mass for a 24-component base. `routing.mjs` (three-less) computes runs rather than drawing lines: support spacing falls out of the deflection limit δ = 5wL⁴/(384EI) ≤ L/360 and therefore scales as g^(−1/3) (measured ratio 1.319297 vs expected 1.319297); sag is a catenary, not a parabola (Newton closes to 1e−14, analytic arc length matches numerical integration to 8 digits); expansion-loop count comes from ΔL = αLΔT; fluid density comes from the band, since assuming water tripled a gas line's mass. Declared line mass must agree with the computed one within 12%. Every component also carries a TECHNICAL SPEC (part number, material, power, pressurised volume, radiation shielding in g/cm², operating band, fastener callout, heat path, ISRU rate) and FOUR budgets that must close: **power** in three cases — the real question is not daylight but the dust storm, where panels drop to 20%, greenhouse and ISRU shed, and the critical 5.81 kW must ride on fission alone (45% margin); **thermal** 18.9 kW capacity vs 11.4 kW, computed from declared fin area via εσ(T⁴ − environment backload) — omitting the backload showed 24.4 kW; **volume** 298.6 m³ habitable, 74.7 m³ per crew, transit and production volume excluded; **ISRU** 5.2 kg/day O₂ against 3.36 kg/day breathed, with Sabatier's water/methane ratio checked against stoichiometry (2.250 measured vs 2.246). Visual depth comes from `hab-detail.mjs`, a shared greeble kit where every item has a reason: Ø19 mm handrails at 57 mm standoff (a gloved hand must fit), handwheel hatches, triple-pane viewports with sliding covers, keyed umbilical panels, cable trays, legible placards, MLI standoffs, footpads, beacons. Shells use torispherical domes — drawn as hemispheres, a 7.6 m module kept only 3.2 m of barrel and read as a blimp. `scripts/validate-habitat.mjs` (106 checks) |
