...
import { palette, backdrop, polyline, marker, label, title, tag, Entrance, reveal, staticMode, rgba, starfield, sun, planet, glow } from '../core/lab-scene.mjs';
...
export async function mountTisserand(host, options = {}) {
  ...
  function drawGraph() {
    ...
    { /* yasak bölge r_p > r_a: köşegenin üstü sönük dolgu */ const lo_ = Math.pow(10, Math.max(xlo, ylo)) * AU, hi_ = Math.pow(10, Math.min(xhi, yhi)) * AU; ctx.save(); ctx.beginPath(); ctx.rect(pad.l, pad.t, pw, ph); ctx.clip(); ctx.fillStyle = 'rgba(255,255,255,.035)'; ctx.beginPath(); ctx.moveTo(X(lo_), Y(lo_)); ctx.lineTo(X(hi_), Y(hi_)); ctx.lineTo(pad.l, pad.t); ctx.closePath(); ctx.fill(); ctx.restore(); label(ctx, 'r_p > r_a: tanımsız', pad.l + 8, pad.t + ph * .2, P, { mono: false, size: 10, color: 'rgba(255,255,255,.3)' }); }
    ...
    for (const p of cfg.planets) { const Pl = PLANETS[p]; ctx.strokeStyle = Pl.color; ctx.globalAlpha = .08; ctx.lineWidth = 9; ctx.beginPath(); ctx.moveTo(pad.l, Y(Pl.a)); ctx.lineTo(pad.l + pw, Y(Pl.a)); ctx.moveTo(X(Pl.a), pad.t); ctx.lineTo(X(Pl.a), pad.t + ph); ctx.stroke(); ctx.lineWidth = 1; ctx.globalAlpha = .35; ctx.beginPath(); ctx.moveTo(pad.l, Y(Pl.a)); ctx.lineTo(pad.l + pw, Y(Pl.a)); ctx.moveTo(X(Pl.a), pad.t); ctx.lineTo(X(Pl.a), pad.t + ph); ctx.stroke(); ctx.globalAlpha = 1; glow(ctx, X(Pl.a), Y(Pl.a), 14, Pl.color, .6); marker(ctx, X(Pl.a), Y(Pl.a), 3, Pl.color, { ring: false }); ctx.fillStyle = Pl.color; ctx.font = '600 10.5px Inter, sans-serif'; ctx.fillText(Pl.label, X(Pl.a) + 4, Y(Pl.a) - 4);
    ...
    plan.legs.forEach((l, i) => { if (!l.orbit) return; const col = legColor(i); const pl = Math.min(1, Math.max(0, pLeg * plan.legs.length - i)); if (pl <= 0) return; ctx.save(); ctx.globalAlpha = pl;
      ...
      const x = X(l.orbit.ra), y = Y(l.orbit.rp); glow(ctx, x, y, 16, col, .55); ctx.fillStyle = col; ctx.beginPath(); ctx.arc(x, y, 5, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = P.ink; ctx.font = '600 10.5px Inter, sans-serif'; ctx.fillText(`${i + 1}: ${PLANETS[l.from].label}→${PLANETS[l.to].label}`, x + 8, y + 4);
  ...
  function drawHelio() {
    const cv = plots.helio, ctx = cv.getContext('2d'), W = cv.clientWidth, Hh = cv.clientHeight; ctx.setTransform(dpr, 0, 0, dpr, 0, 0); backdrop(ctx, W, Hh, { canvas: P.canvas, grid: 36, gridAlpha: .025 }); starfield(ctx, W, Hh, { seed: 17, n: 110, alpha: .45 }); const pLeg = entrance.progress('legs');
    ...
    cfg.planets.forEach((p, pi) => { const Pl = PLANETS[p]; if (Pl.a > rmax) return; ctx.strokeStyle = Pl.color; ctx.globalAlpha = .4; ctx.beginPath(); ctx.arc(X(0), Y(0), Pl.a * sc, 0, Math.PI * 2); ctx.stroke(); ctx.globalAlpha = 1;
      /* gezegen diski: yörüngesinde, Güneş'e dönük terminatörle (konum gösterim amaçlı — fazlama yok) */ const a0 = -.9 + pi * .55, px = X(Pl.a * Math.cos(a0)), py = Y(Pl.a * Math.sin(a0)); planet(ctx, px, py, p === 'jupiter' || p === 'saturn' ? 6.5 : 4, { color: Pl.color, sunDir: [X(0) - px, Y(0) - py], rings: p === 'saturn' ? { tilt: .3 } : null, atmosphere: p === 'earth' ? '#6fb4ff' : null }); label(ctx, Pl.label, px + 8, py + 4, P, { mono: false, size: 10, color: rgba(Pl.color, .85) }); });
