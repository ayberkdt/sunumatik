...
export async function mountTisserand(host, options = {}) {
  ...
  figure.innerHTML = `
    <style>
      ...
      .tg__hud{padding:8px 12px;border-top:1px solid var(--color-rule,#3a3c42);font-size:12px;color:var(--color-muted,#9a938a);overflow:auto;max-height:58%;}
      ...
      .tg__hud td{text-align:right;padding:2px 4px;font-family:var(--font-mono,'JetBrains Mono',ui-monospace,monospace);font-size:11.5px;color:var(--color-ink,#e9e4d8);white-space:nowrap;} .tg__hud td.bad{color:var(--color-data-2,#d78f6c);}
      .tg__hud .note{margin-top:6px;font-size:11px;letter-spacing:.03em;}
    </style>
    <div class="tg__cell"><canvas class="p" data-plot="graph" aria-label="Tisserand grafiği"></canvas></div>
    <div class="tg__side">
      <div class="tg__cell"><canvas class="p" data-plot="helio" aria-label="Güneş-merkezli yörüngeler"></canvas></div>
      <div class="tg__hud" role="status"><table><thead><tr><th>bacak</th><th>v∞ giriş</th><th>α → α′</th><th>|Δα| / δ_max</th><th>r_p / r_a (AU)</th><th>T (yıl)</th><th>v∞ varış</th></tr></thead><tbody data-rows></tbody></table><div class="note" data-note></div></div>
    </div>`;
  ...
  function drawGraph() {
    const cv = plots.graph, ctx = cv.getContext('2d'), W = cv.clientWidth, Hh = cv.clientHeight; ctx.setTransform(dpr, 0, 0, dpr, 0, 0); ctx.fillStyle = P.canvas; ctx.fillRect(0, 0, W, Hh);
    ...
    for (const p of cfg.planets) { const Pl = PLANETS[p]; ctx.strokeStyle = Pl.color; ctx.globalAlpha = .35; ctx.beginPath(); ctx.moveTo(pad.l, Y(Pl.a)); ctx.lineTo(pad.l + pw, Y(Pl.a)); ctx.moveTo(X(Pl.a), pad.t); ctx.lineTo(X(Pl.a), pad.t + ph); ctx.stroke(); ctx.globalAlpha = 1; ctx.fillStyle = Pl.color; ctx.font = '600 10.5px Inter, sans-serif'; ctx.fillText(Pl.label, X(Pl.a) + 4, Y(Pl.a) - 4);
      for (const c of contours[p]) { ctx.strokeStyle = Pl.color; ctx.globalAlpha = .55; ctx.lineWidth = 1; ctx.beginPath(); let first = true; for (const o of c.pts) { if (o.ra === Infinity || o.rp <= 0) continue; first ? ctx.moveTo(X(o.ra), Y(o.rp)) : ctx.lineTo(X(o.ra), Y(o.rp)); first = false; } ctx.stroke(); ctx.globalAlpha = 1; const tip = c.pts[Math.floor(c.pts.length * .5)]; if (tip && tip.ra < Infinity) { ctx.font = '9px ui-monospace, monospace'; ctx.fillStyle = Pl.color; ctx.fillText(`${c.vinf}`, X(tip.ra) + 2, Y(tip.rp) - 2); } } }
    /* dizi bacakları */
    plan.legs.forEach((l, i) => { if (!l.orbit) return; const col = legColor(i);
      ...
      const prev = plan.legs[i - 1]; if (prev && prev.orbit) { ctx.strokeStyle = col; ctx.setLineDash([3, 3]); ctx.beginPath(); ctx.moveTo(X(prev.orbit.ra), Y(prev.orbit.rp)); ctx.lineTo(x, y); ctx.stroke(); ctx.setLineDash([]); } });
    ctx.restore();
  }
  function drawHelio() {
    const cv = plots.helio, ctx = cv.getContext('2d'), W = cv.clientWidth, Hh = cv.clientHeight; ctx.setTransform(dpr, 0, 0, dpr, 0, 0); ctx.fillStyle = P.canvas; ctx.fillRect(0, 0, W, Hh);
    ...
    ctx.fillStyle = P.muted; ctx.font = '10.5px Inter, sans-serif'; ctx.fillText('güneş-merkezli bacak yörüngeleri (eş-düzlem, fazlama yok)', 10, 14);
    ...
    plan.legs.forEach((l, i) => { if (!l.orbit) return; const o = l.orbit, col = legColor(i); ctx.strokeStyle = col; ctx.lineWidth = 1.5; ctx.beginPath(); for (let k = 0; k <= 240; k++) { const nu = 2 * Math.PI * k / 240, r = o.a * (1 - o.e * o.e) / (1 + o.e * Math.cos(nu)); const x = r * Math.cos(nu + i * .35), y = r * Math.sin(nu + i * .35); k ? ctx.lineTo(X(x), Y(y)) : ctx.moveTo(X(x), Y(y)); } ctx.stroke(); ctx.fillStyle = col; ctx.font = '10px Inter, sans-serif'; ctx.fillText(`${i + 1}`, X(o.ra * Math.cos(Math.PI + i * .35)) + 4, Y(o.ra * Math.sin(Math.PI + i * .35))); });
    ctx.fillStyle = P.muted; ctx.font = '10px Inter, sans-serif'; ctx.fillText('elipsler bacak numarasıyla; yönelimler görsel ayrım için kaydırılmıştır', 10, Hh - 8);
  }
  ...
  rebuild(); resize();
  return { get plan() { return plan; }, get config() { return { ...cfg }; }, sequences: SEQUENCES, planets: PLANETS, set(c) { cfg = { ...cfg, ...c }; rebuild(); draw(); }, dispose() { ro.disconnect(); figure.remove(); } };
}
