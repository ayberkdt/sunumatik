...
export async function mountThreeBody(host, options = {}) {
  ...
  const resetPanel = p => { p.st = Float64Array.from(p.entry.state()); p.t = 0; p.hist = []; };
  ...
  const stepPanel = (p, dt) => {
    ...
    for (let s = 0; s < sub; s++) { advance(p.st, p.m, h, p.entry.chaotic ? { hMax: 1e-3, k: 2e-3 } : { hMax: 2e-3, k: 4e-3 }); p.t += h; p.hist.push([p.t, p.st[0], p.st[1], p.st[2], p.st[3], p.st[4], p.st[5]]); }
    const keep = p.t - spanOf(p) * 1.5; let cut = 0; while (cut < p.hist.length - 2 && p.hist[cut + 1][0] < keep) cut++; if (cut) p.hist.splice(0, cut);
    return dp;
  };
  ...
  function drawTrails() {
    tctx.setTransform(dpr, 0, 0, dpr, 0, 0); tctx.clearRect(0, 0, W, H); tctx.globalCompositeOperation = 'source-over'; tctx.lineCap = 'round'; tctx.lineJoin = 'round';
  ...
  function draw() {
    ...
    ctx.filter = `blur(${(2.5 * fb).toFixed(1)}px)`; ctx.globalAlpha = .55; ctx.drawImage(trailCv, 0, 0); ctx.filter = `blur(${(10 * fb).toFixed(1)}px)`; ctx.globalAlpha = .45; ctx.drawImage(trailCv, 0, 0); ctx.filter = 'none'; ctx.globalAlpha = .9; ctx.drawImage(trailCv, 0, 0); ctx.globalAlpha = 1; ctx.restore();
    panels.forEach((p, i) => {
  ...
  function advanceFrame(dt) { tickView(dt); const ds = dt * timeline.speed; timeline.t += ds; for (const p of panels) stepPanel(p, ds); draw(); }
