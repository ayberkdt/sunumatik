...
export async function mountGeoSk(host, options = {}) {
  ...
  figure.innerHTML = `
    <style>
      ...
      .gk__hud{padding:12px 16px 10px;border-top:1px solid var(--lab-rule,#3a3c42);}
    ...
    <div class="gk__left">
      <div class="gk__cell" data-lab-reveal="fade"><canvas class="p" data-plot="lon" aria-label="Boylam ivmesi ve yıllık ΔV"></canvas></div>
      <div class="gk__cell" data-lab-reveal="fade"><canvas class="p" data-plot="drift" aria-label="Serbest sürüklenme"></canvas></div>
      <div class="gk__hud lab-hud" role="status" data-lab-reveal>
        <div class="lab-hud__hero">
          <div><span class="k">ΔV doğu–batı / yıl</span><span class="v hi" data-h="ew">—</span></div>
          <div><span class="k">ΔV kuzey–güney / yıl</span><span class="v hi" data-h="ns">—</span></div>
          <div><span class="k">Ömür ΔV</span><span class="v" data-h="life">—</span></div>
          <div><span class="k">Yakıt kimyasal / elektrikli</span><span class="v" data-h="prop">—</span></div>
        </div>
        <dl>
    ...
    <div class="gk__right">
      <div class="gk__cell" data-lab-reveal="fade"><canvas class="p" data-plot="inc" aria-label="Eğiklik vektörü"></canvas></div>
      <div class="gk__cell" data-lab-reveal="fade"><canvas class="p" data-plot="ecc" aria-label="Eksantriklik vektörü"></canvas></div>
      <div class="gk__cell" data-lab-reveal="fade"><canvas class="p" data-plot="budget" aria-label="Yıllık bütçe"></canvas></div>
    </div>`;
  ...
  const P = palette(figure); const legendEl = figure.querySelector('[data-legend]'); if (legendEl) legendEl.innerHTML = `<span><i style="background:${P.ink}"></i>yıllık ΔV(λ)</span><span><i style="background:${P.data1}"></i>boylam ivmesi λ̈ · eğiklik izi</span><span><i style="background:#8fd39a"></i>kararlı nokta</span><span><i style="background:${P.data2}"></i>kararsız nokta · SRP e çemberi</span>`;
  const entrance = new Entrance({ frame: { at: 0, dur: .4 }, curves: { at: .2, dur: 1.2 }, marks: { at: 1.1, dur: .5 } }, { onFrame: () => draw() });
  ...
  function writeHud() {
    ...
    H.ew.innerHTML = `${nf2.format(M.ewPerYear)}<span class="u">m/s</span>`; H.ns.innerHTML = `${nf1.format(M.nsPerYear)}<span class="u">m/s</span>`; H.di.textContent = `${nf3.format(ns.diPerYear)}°`; H.lib.textContent = `${nf1.format(lib)} yıl`;
    H.e.textContent = `${srp.eMax.toExponential(2)} (A/m 0,04)`; H.c.textContent = `${COEFF.C22.toExponential(3)} · ${COEFF.S22.toExponential(3)}`;
    H.life.innerHTML = `${nf0.format(M.lifetimeDv)}<span class="u">m/s · ${cfg.years} yıl</span>`; H.prop.innerHTML = `${nf0.format(M.propChem)} / ${nf0.format(M.propEp)}<span class="u">kg</span>`;
  }
  let dpr = 1;
  function frame(cv, text) { const ctx = cv.getContext('2d'), W = cv.clientWidth, Hh = cv.clientHeight; ctx.setTransform(dpr, 0, 0, dpr, 0, 0); backdrop(ctx, W, Hh, { canvas: P.canvas }); title(ctx, text, 12, 15, P); const pad = { l: 52, r: 14, t: 22, b: 20 }; return { ctx, W, Hh, pad, pw: W - pad.l - pad.r, ph: Hh - pad.t - pad.b }; }
  function drawLon() {
  ...
  function drawEcc() {
    ...
    ctx.strokeStyle = 'rgba(255,255,255,.12)'; ctx.beginPath(); ctx.moveTo(pad.l, cy); ctx.lineTo(pad.l + pw, cy); ctx.moveTo(cx, pad.t); ctx.lineTo(cx, pad.t + ph); ctx.stroke();
    polyline(ctx, tr.map(p => [X(p.ex), Y(p.ey)]), { progress: entrance.progress('curves'), color: P.data2, width: 1.4 });
  ...
  return { replay() { entrance.start(); }, get model() { return { budget: M, equilibria: eq, libration: lib, ns, srp, drift }; }, get config() { return { ...cfg }; }, set(c) { cfg = { ...cfg, ...c }; rebuild(); draw(); }, dispose() { ro.disconnect(); figure.remove(); } };
}
