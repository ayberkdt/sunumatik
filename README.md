...
| [`physical_rigs/`](presets/physical_rigs) | Mekanizma sürücüsü (three'siz çekirdek): craft parça sözleşmesindeki adlı eklemleri gerçek aktüatör gibi sürer — kapalı form kritik sönüm (kadans-bağımsız), hız sınırı, sınır açısı, tek yönlü strok, yaylı açılım, rotor rpm rampası + stroboskop kuralı, kademe ayrılması; saf çözücüler rocker-bogie (diferansiyel), Ackermann, tekerlek ω = v/r + kayma; araç başına koreografi programları (saf f(t)); `scripts/validate-rigs.mjs` 44 denetim. Araç vitrininde Mekanizma/Etiketler anahtarları. |
