...
| [`craft_blocks/`](presets/craft_blocks) | Estetik parametrik araç kütüphanesi: orbiter, iniş aracı, 2 kademeli roket, CubeSat, kapsül + 2. dalga: Starship, gezgin (rover), Mars helikopteri, derin uzay sondası — donmuş eksen/palet sözleşmesiyle tüm bloklar birleşir. Her ayrıntı bir mekanizma ya da kısıt anlatır (`userData.notes.why`); **parça sözleşmesi** (`userData.rig`: adlı eklemler, eksen, sınır, hız — SADA, gimbal, bacak strok'u, rocker-bogie pivot ağacı, kapaklar) physical_rigs F0. `craft-effects.mjs` sinematik ateşleme sistemi (mach elmaslı alevler, ateşleme flaşı). |
| [`stage_separation/`](presets/stage_separation) | **İki kademeli fırlatma + KATI YAKITLI İTİCİLER, sinematik yakalama için.** Zaman çizelgesi elle yazılmaz: `launch_ascent/ascent-model.mjs` entegre yörüngesinden TÜRETİLİR (Max-Q, MECO, ayrılma, SES, kapak, SECO). Kademe ayrılması bir animasyon değil bir DEVİRDİR — itici sahneye bırakılır, kafes kanatçıklarını açar ve kendi balistiğini yaşar (`physical_rigs/jettison.mjs`, kapalı form: her ana adım atmadan gidilir). İki yan itici (SRB) kalkış itkisini verir, 69 s'de tükenir ve 71 s'de takla atarak atılır — kısılamayan/söndürülemeyen katı itici erken biter, boş kütlesi taşınmaz. Kapak iki yarım hâlinde açılıp dönerek uzaklaşır. Kamera yönetmeni kesme noktalarını olaylardan alır; `?sinema=1` arayüzü kaldırır ve 2.39:1 maskesi indirir. |
| [`mechanism_lab/`](presets/mechanism_lab) | **Aksam laboratuvarı**: bir mekanizmayı parça parça inceleme tezgâhı. 16 aksam (rocker-bogie, Ackermann direksiyon, çıtalı tekerlek, robot kol, pan-tilt direk, az-el çanak, bal peteği bacak stroku, TVC gimbal, kafes kanatçık, başlık menteşesi, SRB ayrılması, yaylı küpsat açılımı, kapsül kapağı, eş eksenli rotorlar, Starship flapleri, SADA tamburu). Her biri için: **patlatılmış görünüm** (parçalar montaj doğrultusunda ayrılır), **kinematik iskelet** (pivotlar, dönme ekseni okları, bağlantı çizgisi), **hareket zarfı** (sürülen düğümün en uç parçasının taradığı GERÇEK yol), eklem tablosu (tür, eksen, sınır, hız), elle sürülen eklem sürgüleri ve künye: yöneten bağıntı · neden böyle · hangi doğrulama sınıyor. Serbestlik derecesi elle yazılmaz, rig haritasından türetilir. |
| [`satellite_integration/`](presets/satellite_integration) | **Uydu entegrasyonu**: bir uydunun neye benzediğini değil NASIL KURULDUĞUNU anlatan patlatılmış görünüm. 33 parça türü, 43 adet, 8 alt sistem, 7 montaj adımı; her parçanın satırında nereye cıvatalandığı, hangi arayüzle (cıvata/kızak/ayırma/akışkan/ısıl/elektrik/menteşe), kaçıncı adımda geldiği, kaç kilo olduğu ve NEDEN VAR OLDUĞU yazılı. Montaj sırası keyfi değil: tank panel kapanmadan girer, ekipman panele tezgâhta takılır, MLI en son gelir çünkü altındaki cıvatalara bir daha ulaşılamaz. Patlatma yönü montajın tersidir ve parçanın ebeveynine göre konumundan türetilir. Kütle bütçesi toplanır (709,4 kg kuru + 415 kg itici = 1124,4 kg) ve kütle merkezi ayırma halkası ekseninden 32,7 mm sapar — fırlatıcı arayüz sınırının içinde. Her parçada ayrıca TEKNİK KÜNYE var: parça numarası (SD-XXX-000), malzeme, güç (üretim negatif), veri hızı, çalışma sıcaklığı bandı, bağlantı elemanı (cıvata sayısı ve torku), ısı yolu ve nitelik beyanı. Bu sayılar süs değil, toplandıklarında KAPANMAK zorunda: güç 2900 W üretim / 2054 W tüketim (%29,2 pay), veri 420 Mbps ham ÷ 4 sıkıştırma = 105 Mbps ≤ 150 Mbps indirme, ısıl 2504 W radyatör kapasitesi ≥ 1934 W atılacak (%22,8 pay). Kapasite künyeden okunmaz, radyatörün beyan edilen alanından εσT⁴ ile HESAPLANIR — künyeye elle yazılan 340 W, aynı panelin gerçekte 1252 W attığını gizliyordu. `scripts/validate-satellite.mjs` 57 denetim. |
| [`exploded_view/`](presets/exploded_view) | **Patlatılmış görünüm — çok cisimli**: tek bir montaj çekirdeği (`core/assembly.mjs`, three görmez) ve onu sahneye uygulayan ince bir bağlayıcı (`core/exploded-view.mjs`). Patlatma yönü rastgele bir dağılma değil: her parça BAĞLANDIĞI parçadan dışarı, montajın tersi doğrultuda çıkar; aynı doğrultudakiler yön üzerindeki izdüşümlerine göre paketlenir, böylece hiçbiri diğerini geçmez (derinliğe göre paketleyen ilk sürümde yakıt ile kapak k=0,5'te 16 mm'ye kadar yaklaşıyordu). Kök yerinde durur, yoksa bütün cisim ötelenir. Dört kip (montaj/eksenel/ışınsal/katmanlı), adım oynatımı, kesit ve seçim. İki adaptör: `adapters/catalog.mjs` beyan edilmiş katalogları alır (uydu ve yüzey üssü aynı biçimde), `adapters/scene.mjs` HERHANGİ bir three nesnesinden montaj grafiği türetir — kütle sınır hacmiyle TAHMİN edilir, `tahmini: true` ile damgalanır ve arayüzde öyle gösterilir; tek köklü olmayan sahneye kütlesi sıfır sentetik bir kök üretilir. Etiketler gövdenin üstüne basılmaz: iki kenar oluğuna alınır, ölçülen yüksekliklerine göre dikeyde ayrıştırılır ve kılavuz çizgisiyle parçalarına bağlanır. Hareket katmanı ayrı ve yine SAF: `core/exploded-motion.mjs` three görmez, kademe (dış parçalar önce kalkar), smootherstep yumuşatma, ayrılma açısı, hız ve seçim nabzı (k, parça)'nın kapalı formlarıdır — integratör yok, kare başına durum yok, bu yüzden kaydırağı geri sürmek tam olarak geri izler. Her parça ayrılırken gidiş yönüne DİK bir eksende hafifçe döner ve **varışta tam olarak beyan edilmiş duruşuna döner** — patlatılmış hâl bir çizimdir, eğik kalan parça onu okunmaz yapar. Parça başına değişim karma edilmiş kimlikten gelir, `Math.random` yok; karmaya MurmurHash3 sonlandırıcısı eklendi çünkü FNV-1a tek başına benzer kimlikleri 0,01 aralığına sıkıştırıyordu (χ² 128,9 iken %1 eşiği 21,67; şimdi 8,0). Tezgâh dönüşü, vurgu ve yumuşak kamera geçişi vitrinde. Sayfa dili İngilizcedir. `scripts/validate-assembly.mjs` 52 ve `scripts/validate-exploded-motion.mjs` 33 denetim. |
| [`habitat_blocks/`](presets/habitat_blocks) | **Habitat blokları**: dört mürettebatlı bir yüzey üssü — 26 bileşen, 8 sistem, 7 kurulum adımı. Katalog biçimi `sat-parts.mjs` ile BİREBİR aynıdır, bu yüzden üs `exploded_view`'da hiçbir özel durum gerektirmeden patlatılır. Ortam kuralı bağlayıcı: Ay'da MOXIE (atmosferde CO₂ yok) ve rüzgâr ölçer (ölçülecek rüzgâr yok) REDDEDİLİR, gerekçesi ve önerisiyle, ve 428 kg kütle bütçesinden düşer. Hat yönlendirme (`routing.mjs`, three'siz) hesaplanır: mesnet aralığı sarkma sınırından çıkar (δ = 5wL⁴/(384EI) ≤ L/360) ve yerçekimiyle g^(−1/3) gibi ölçeklenir — Ay'da 13,3 m, Mars'ta 10,1 m, ölçülen oran 1,319297 (beklenen 1,319297); sarkma zincir eğrisidir, parabol değil (Newton 1e−14'te kapanıyor, yay uzunluğu analitik = sayısal 8 hanede); genleşme ilmeği sayısı ΔL = αLΔT'den gelir. Beyan edilen hat kütlesi ile hesaplanan %12 içinde tutmak zorundadır. Her bileşen ayrıca TEKNİK KÜNYE taşır: parça numarası (HB-XXX-000), malzeme, güç, basınçlı hacim, radyasyon koruması (g/cm²), çalışma sıcaklığı, bağlantı elemanı, ısı yolu ve ISRU debisi. Dört bütçe kapanmak zorunda ve denetim kapanışı sınar: **güç** üç durum için ayrı — gündüz %10, gece %42, toz fırtınasında %45 pay; asıl soru "gündüz yetiyor mu" değil, panel %20'ye düşüp sera ile ISRU kapandığında kritik 5,81 kW yükün yalnız fisyonla taşınıp taşınmadığıdır. **ısıl** 18,9 kW kapasite ≥ 11,4 kW yük, kapasite kanat alanından εσ(T⁴ − ortam geri ışıması) ile hesaplanır — ortam terimi atlanırsa kapasite 24,4 kW görünüyordu. **hacim** 298,6 m³ yaşanabilir, kişi başı 74,7 m³ (geçiş ve üretim hacmi sayılmaz). **ISRU** 5,2 kg/gün O₂ üretimi ≥ 3,36 kg/gün solunum; Sabatier'in su/metan oranı stokiyometriden sınanır (ölçülen 2,250, beklenen 2,246). Görsel taraf `hab-detail.mjs` ayrıntı kitinden gelir: Ø19 mm el rayı 57 mm açıklıkla, çarklı basınç kapağı, üç camlı pencere ve sürgülü kapak, anahtarlı konnektör paneli, kablo tavası, okunur uyarı levhası, MLI ayırıcıları, ayak pabucu ve seyrüsefer feneri — her biri bir gereksinimden doğar. Kabuk torisferik kapaklıdır: yarım küreyle çizdiğimde 7,6 m modülün yalnız 3,2 m'si silindir kalıyor ve modül zeplin gibi duruyordu. `scripts/validate-habitat.mjs` 106 denetim. |
| [`light_blocks/`](presets/light_blocks) | **Işık fiziği**: aynı lamba dört ortamda yan yana — fark içeriğin kendisi. `core/light-math.mjs` rengin üç meşru kaynağını tanımlar (sıcaklık: Planck → CIE 1931 → doğrusal sRGB, ölçülen renklilik yayımlanmış eğriye Δxy < 0,0016; kimya: emisyon hatları; yansıma: albedo × aydınlatan) ve ortam kuralını BAĞLAYICI kılar: vakumda açık alev yanmaz, ışık huzmesi görünmez, duman asılı kalmaz — her ret nedenini ve önerisini söyler. `light-rig.mjs` Güneş'i gerçek aydınlatmasıyla kurar (Ay 127 000 lux), dolguyu HESAPLAR (regolit sıçraması E·ρ·sin(irtifa)/π, Dünya ışığı ~12 lux) ve AmbientLight'ı hiç kurmaz. Far kandela ile, IES benzeri profil, zemin aydınlatması E = I/d² olarak ekranda ölçülü. `scripts/validate-light.mjs` 57 denetim. |
| [`life_signs/`](presets/life_signs) | **Nefes kütüphanesi**: bir sahnenin canlı görünmesi hareket EKLEMEKLE olmaz, gerçek bir mekanizmanın gerçek BOŞTA davranışını göstermekle olur. `core/life-signs.mjs` üç'süz primitifler: φ türevi osilatör bankası, iki ölçüşmez sinüsün çarpımı olan pürüz zarfı (**tek sinüs asla**), Poisson olay programı, KAPALI FORM kritik sönümlü saccade (hız sınırı ω = min(ωMax, rateMax·e/|e₀|) ile aktüatör fiziğinden gelir), sürekli segment zinciri, bakış programı, karşı-nefes. `life_signs/` katalogda 22 satır: 14'ü çalışır (direk bakışı, durum feneri, HGA takibi, panel Güneş takibi, RCS pufu, araç solunumu, tutunma, kamera nefesi, belgesel yayı, engel lambası, kablo salınımı, yıldız parıldaması, göktaşı, HUD oturması), 1'i **reddedilmiştir** (gerçek gezginde kol boştayken kilitlidir), 7'si bağlanacağı kütüphaneyi bekler. `budget.mjs` dikkat bütçesini zorlar (Σ salience ≤ 1,0, en çok 3 fark edilir davranış, kamera yaklaştıkça bütçe daralır) ve **yasak sınıfları reddeder**: arazi, alan çizgileri, yörünge izi, sabit ışıkta gölge, boşta HUD sayısı nefes almaz. `bind.mjs` kanalları three'ye İKİNCİL olarak yazar, rayı sürmez. `scripts/validate-life-signs.mjs` 34 denetim. |
| [`physical_rigs/`](presets/physical_rigs) | **Mekanizma sürücüsü + araziye bağlı sürüş vitrini.** Çekirdek three'siz: adlı eklemleri gerçek aktüatör gibi sürer (kapalı form kritik sönüm, hız sınırı, sınır açısı, tek yönlü strok, yaylı açılım, rotor rpm rampası + stroboskop kuralı → bulanıklık diski, kademe ayrılması). Saf çözücüler: rocker-bogie (gövde yunuslaması = iki rockerın ortalaması), Ackermann (yerinde dönüş), tekerlek ω = v/(r(1−s)) + kayma. `terrain-treadmill.mjs` KESİNTİSİZ dönen zemin: yükseklik z'de periyodik yazılır, döşeme sınırında ne yükseklik ne eğim atlar; döngü tekerleğin tam sayı devrinden kapanır (travel = 2π·r_dış·N). `ground-treadmill.mjs` döşeli örgü, periyodik prosedürel doku, kaya alanı, tekerlek izi şeridi, temas gölgesi. Vitrin: gezgin gerçek arazide sürer, ölçülen temas artığı 1,9 mm. `scripts/validate-rigs.mjs` 64 denetim. |
...
| [`site_plan/`](presets/site_plan) | **Saha yerleşimi**: bir yüzey üssünün planını KURAL GÜDÜMLÜ çözer. 11 adlandırılmış kural (reaktör ≥ 1 km, iniş pisti ≥ 500 m ve ±30°'lik ejecta konisinde yapı yok, yakıt ↔ O₂ ≥ 50 m, radyatör Güneş'e sırt, güneş tarlası gölgede değil, garaj ≤ 150 m, ISRU rüzgâr üstü — yalnız Mars, eğim eşiği, çakışma yok); sert kural adayı eler, yumuşak kural ağırlıkla puanlar, çözülemeyen modül 'zorlandı' olarak BİLDİRİLİR. Her kuralın ölçülen değeri hedefiyle yan yana yazılır; modüller sürüklenerek elle konabilir ve kurallar anında yeniden ölçülür. Çözümün ürettiği `clearings` doğrudan `terrain_blocks`'a verilir: saha çokgenleri ve 24 m'lik koridorlar araziyi gerçekten tesviye eder (3B önizleme). Çözücü `presets/terrain_blocks/site-plan.mjs` three'siz ve deterministiktir; doğrulama `scripts/validate-site.mjs`. |
| [`terrain_blocks/`](presets/terrain_blocks) | **Arazi kütüphanesi** (craft_blocks'un arazi ikizi, mount'suz): olgu katmanlı yükseklik alanı — Pareto boy dağılımlı krater alanı (aşınma yaşı, karmaşık krater), havza masifleri, mare sırtları, kanyon, barchan kumul (yalnız Mars), mesa, rille, çökme çukuru, fBm, mikro kabartma; gerçek yarıçaplı sagitta, 35° durma açısına termal erozyonla eğim kırpma (sert kaya muaf), gezgin pedi / sürüş koridoru / saha kapıları; profiller Ay mare · yayla · kutup kenarı, Mars ova · kanyon, genel kayalık; Lambert regolit malzemesi (eğim/yükseklik albedo, dört-tap detay merdiveni, bölge albedosu, overlay kancası); mavi-gürültü kaya/çakıl; **sorgu API'si** `height / normal / slopeDeg / contact / raycastDown / nearestRocks` (tekerlek, ayak, gölge tüketir). Vitrin: profil, tohum, Güneş irtifası, yükseklik/eğim haritası. Node doğrulaması `scripts/validate-terrain.mjs`. Tasarım: [docs/terrain-system-plan.md](docs/terrain-system-plan.md). |
