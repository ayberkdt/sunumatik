...
| [`stage_separation/`](presets/stage_separation) | **İki kademeli fırlatma, sinematik yakalama için.** Zaman çizelgesi elle yazılmaz: `launch_ascent/ascent-model.mjs` entegre yörüngesinden TÜRETİLİR (Max-Q, MECO, ayrılma, SES, kapak, SECO). Kademe ayrılması bir animasyon değil bir DEVİRDİR — itici sahneye bırakılır, kafes kanatçıklarını açar ve kendi balistiğini yaşar (`physical_rigs/jettison.mjs`, kapalı form: her ana adım atmadan gidilir). Kapak iki yarım hâlinde açılıp dönerek uzaklaşır. Kamera yönetmeni kesme noktalarını olaylardan alır; `?sinema=1` arayüzü kaldırır ve 2.39:1 maskesi indirir. |
